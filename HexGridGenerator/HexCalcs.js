function findHexWithSideLengthZAndRatio(sideLength, whRatio = HT.Hexagon.Static.whRatio)
{
	var z = sideLength;
	var r = whRatio;
...
function findHexWithWidthAndHeight(hexWidth, hexHeight)
{
	var width = hexWidth;
...
function drawHexGrid(screenW = 800, screenH = 600)
{
...
function getHexGridZR(sideLength, whRatio, screenW, screenH)
{
	findHexWithSideLengthZAndRatio(sideLength, whRatio);
	return drawHexGrid(screenW, screenH);
}

function getHexGridWH(hexWidth, hexHeight)
{
	findHexWithWidthAndHeight(hexWidth, hexHeight);
	return drawHexGrid();
}

function changeOrientation(hexOrientationNormal)
{
...
function debugHexZR(sideLength, whRatio)
{
	findHexWithSideLengthZAndRatio(sideLength, whRatio);
	addHexToCanvasAndDraw(20, 20);
}

function debugHexWH(hexWidth, hexHeight)
{
	findHexWithWidthAndHeight(hexWidth, hexHeight);
	addHexToCanvasAndDraw(20, 20);
}
